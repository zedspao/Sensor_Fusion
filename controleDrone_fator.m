...
while numel(height_IR) == 0 %Ler infravermelho
height_IR = readHeightIR(r); %Precisa garantir a leitura
end

c = 0.9487; %Fator de correcao da leitura
h = height_IR*c; %Corrige a leitura
...
if (u0 >= 0.2) && (u0 <= 5) %Se a acao de controle atende os limites:
u = u0;
moveup(r,'distance',u,'WaitUntilDone',false) %Sobe o drone
end

if (u0 <= -0.2) && (u0 >= -5) %Se a acao de controle for de polaridade contraria:
u = u0*-1;
movedown(r,'distance',u,'WaitUntilDone',false) %Desce o drone
u = u0;
end
